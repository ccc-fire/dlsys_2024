...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
   ...
   * 将非紧密数组中a的单个条目映射到紧密数组out中
   * 每个线程都将非紧密数组 a 中的一个元素复制到紧密数组 out 的相应位置，从而完成了压缩操作
   */
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;

  /// BEGIN SOLUTION
  if(gid < size) {
    size_t idx_a = 0, idx_out = gid;
    size_t cnt = shape.size - 1;
    while(gid) {
      idx_a += (gid % shape.data[cnt]) * strides.data[cnt];
      gid /= shape.data[cnt];
      cnt--;
    }
    out[idx_out] = a[idx_a + offset];
  }
  /// END SOLUTION
}
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape, 
  ...
  if(gid < size) {
    size_t idx_a = gid, idx_out = 0;
    size_t cnt = shape.size - 1;

    while(gid) {
      idx_out += (gid % shape.data[cnt]) * strides.data[cnt];
      gid /= shape.data[cnt];
      cnt--;
    }
    out[idx_out + offset] = a[idx_a];
  }
...
__global__ void EwiseEqKernel(const scalar_t* a, const scalar_t* b, scalar_t* out, size_t size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if(gid < size) out[gid] = abs(a[gid] - b[gid]) < 1e-6 ? 1 : 0;  // abs 取绝对值
}
...
__global__ void ScalarEqKernel(const scalar_t* a, const scalar_t val, scalar_t* out, size_t size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if(gid < size) {
    out[gid] = abs(a[gid] - val) < 1e-6 ? 1 : 0;
  }
}
...
__global__ void ReduceSumKernel(const scalar_t* a, scalar_t* out, size_t reduce_size, size_t out_size) {
  ...
  if(gid < out_size) {
    size_t offset = gid * reduce_size;
    scalar_t reduce_sum = a[offset];
    for(int i = 1; i < reduce_size; i++) {
      reduce_sum += a[offset + i];
    }
    out[gid] = reduce_sum;
  }
}
...
PYBIND11_MODULE(ndarray_backend_cuda, m) {
  ...
  py::class_<CudaArray>(m, "Array")  // 实际上是一个构造函数
